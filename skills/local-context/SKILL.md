...
1. Project `README.md` or equivalent root documentation — always first when present
2. Nearby architecture or conventions docs in the consuming project
3. Source files matching the research brief's `local_search_targets`
4. Tests or examples that clarify expected patterns

## Workflow

1. Load the project README or the closest equivalent entry document.
...
- Run: `ls README.md` when present to confirm the project entry document is accessible.
...
Research brief lists `local_search_targets: ["src/api/routes", "validation patterns"]`. Load the project README, grep for route files and validation utilities, stop once patterns are clear. Report 3 files found, 2 naming patterns, 1 gap (no existing pagination pattern). This works because search was targeted, not exhaustive.
...
- "I'll skip the project README and go straight to grep."
